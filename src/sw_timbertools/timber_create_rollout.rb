...
module TimberTools
  ...
  def self.rollout_timber(sel)
    load_style() # load the SW rolloout style 
    set_camera()
    page, layer = create_renew_scene(sel) # create a scene and a layer
    draw_instances(sel, layer) # roll away
    page.update # this seems to work
    Sketchup.active_model.active_view.zoom_extents
    page.update
  end
  
  def self.draw_instances(sel, layer)
    model = Sketchup.active_model
    entities = model.active_entities

    tr = Geom::Transformation.rotation([0, 0, 0], [0, 1, 0], 90.degrees)
    ent = entities.add_instance(sel.definition, tr)
    ent.layer = layer
    ...
    entities.add_instance(sel.definition, tr).layer = layer
  ...
  def self.create_renew_scene(sel)
    ...
    page_name = "rollout #{timber_name}"
    
    if model.pages[page_name]
      rollout_page = model.pages[page_name]
    else
      rollout_page = model.pages.add page_name
    end
    model.pages.selected_page = rollout_page # make this active
    

    if model.layers[page_name]
      rollout_layer = model.layers[page_name]
    else
      rollout_layer = model.layers.add page_name
    end
    ...
    layers.each {|layer| rollout_page.set_visibility(layer, false)}
  ...
  def self.set_camera()
      ...
      camera.set([0, -1000, 0], [ 0, 0, 0], [0, 0, 1])
